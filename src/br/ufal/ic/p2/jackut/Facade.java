...
public class Facade {
    ...
    //no teste 7_1 diz que tem erro de conversao entao temq ver isso, vai ser corrigido na refatoraçao
    ...
    public void enviarRecado(Usuario remetente, Usuario destinatario, String mensagem) throws UserNotFoundException {
    ...
    public void adicionarPaquera(String id, String paquera) throws UserNotFoundException, AlreadyEnemyException, ExistentRelantionshipException {
        ...
        if (user.getPaquerasRecebidas().contains(paquerado) || paquerado.getPaquerasRecebidas().contains(user)) {
            this.enviarRecado(user, paquerado, user.getNome() + " é seu paquera - Recado do Jackut.");
            this.enviarRecado(paquerado, user, paquerado.getNome() + " é seu paquera - Recado do Jackut.");
        }
